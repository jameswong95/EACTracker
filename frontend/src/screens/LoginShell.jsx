      ...
      <div className="col grow">
        ...
        <div className="p-5 col gap-4 grow">
          ...
          <div className="box p-4 grow">
            ...
            <div className="tbl" style={{ gridTemplateColumns: '2fr 1fr .8fr 1.2fr 1.2fr' }}>
              {['Project', 'EAC', 'Var', 'Status', 'Last update'].map((c, i) => <div key={i} className="th">{c}</div>)}
              {[
                ['EAC Refresh Programme', '$1.24M', '+1.2%', <span key="s1" className="pill ok tiny"><span className="dot ok" />On Track</span>, '5 May'],
                ['Network Modernisation', '$880K', '-3.1%', <span key="s2" className="pill warn tiny"><span className="dot warn" />At Risk</span>, '2 May · overdue'],
                ['Site Comm. Phase 2', '$2.10M', '-7.4%', <span key="s3" className="pill bad tiny"><span className="dot bad" />Delayed</span>, '28 Apr · overdue'],
                ['Asset Refresh', '$420K', '+0.5%', <span key="s4" className="pill ok tiny"><span className="dot ok" />On Track</span>, '9 May'],
              ].map((r, ri) => r.map((c, ci) => <div key={`${ri}-${ci}`} className="td">{c}</div>))}
            </div>
          </div>
        </div>
      </div>
