      ...
      <div className="flex gap-4 items-stretch">
        ...
        <div className="card card-p grow" style={{ flex: '1.6' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-3 text-xs text-muted" style={{ fontSize: 11 }}>
          ...
          <div className="flex" style={{ borderBottom: '1px solid var(--border)', padding: '0 16px' }}>
            {tabs.map(t => (
              <button key={t} onClick={() => setTab(t.toLowerCase().replace(' ', '-'))}
                style={{
                  padding: '12px 12px 10px', fontSize: 12, fontWeight: tab === t.toLowerCase().replace(' ', '-') ? 600 : 500,
  ...
  return (
    <div className="flex-col gap-2">
      {milestones.map((m) => {
        ...
        return (
          <div key={m._id} style={{ borderBottom: '1px solid var(--border)', borderRadius: isConfirming ? 6 : 0, overflow: 'hidden' }}>
            <div className="flex items-center gap-3" style={{ padding: '8px 0', background: isConfirming ? 'var(--bad-bg)' : '' }}>
              <button
              ...
              <div className="grow">
                <div style={{ fontSize: 13, fontWeight: m.done ? 400 : 600, color: isConfirming ? 'var(--bad-text)' : m.done ? 'var(--text-3)' : 'var(--text)', textDecoration: m.done ? 'line-through' : 'none' }}>{m.name}</div>
                {isConfirming
  ...
  return (
    <div className="flex-col gap-3">
      {risks.map((r) => {
        ...
        return (
          <div key={r._id} className="card" style={{ padding: 14, borderColor: isConfirming ? 'var(--bad)' : undefined, background: isConfirming ? 'var(--bad-bg)' : undefined }}>
