...
// CSS injected once at module load time
...
export function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(() => {
      try { localStorage.setItem(DC_STATE_KEY, JSON.stringify({ sections: state.sections })); } catch {}
    }, 250);
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  const tfKey = 'dc-viewport:' + (typeof location !== 'undefined' ? location.pathname : '/');
