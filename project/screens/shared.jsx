/* shared.jsx — common sketchy UI primitives */
...
// Hand-drawn arrow used for flow connectors
const Arrow = ({dir='right', length=40, color='#3a352f', label}) => {
  const w = dir==='down' || dir==='up' ? 26 : length;
  const h = dir==='down' || dir==='up' ? length : 26;
  const path = dir==='right' ? `M4 ${h/2} Q${w*0.4} ${h/2-3} ${w-8} ${h/2}` :
               dir==='down'  ? `M${w/2} 4 Q${w/2-3} ${h*0.4} ${w/2} ${h-8}` :
               dir==='left'  ? `M${w-4} ${h/2} Q${w*0.6} ${h/2+3} 8 ${h/2}` :
                               `M${w/2} ${h-4} Q${w/2+3} ${h*0.6} ${w/2} 8`;
  const tip = dir==='right' ? `M${w-12} ${h/2-5} L${w-6} ${h/2} L${w-12} ${h/2+5}` :
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:2,flex:'0 0 auto'}}>
      ...
      {label && <div style={{fontFamily:'Caveat',fontSize:13,color:'#6b655e',whiteSpace:'nowrap',marginTop:-4}}>{label}</div>}
    </div>
  );
};

// Stylized line chart placeholder
const LineChart = ({width=300, height=120, points, budgetLine=true, label}) => {
...
const BarChart = ({width=300, height=100, bars, gap=4}) => {
  const data = bars || [
...
// Status pill
const Status = ({status='ok', children}) => {
  const cls = status==='ok'?'ok':status==='warn'?'warn':status==='bad'?'bad':status==='info'?'info':'';
  const label = children || (status==='ok'?'On Track':status==='warn'?'At Risk':status==='bad'?'Delayed':status==='info'?'Completed':'—');
  return <span className={`pill ${cls}`}><span className={`dot ${cls}`}/>{label}</span>;
};

// Faux table
const Table = ({cols, rows, colWidths}) => {
  const tmpl = colWidths || cols.map(()=>'1fr').join(' ');
  return (
    <div className="tbl" style={{gridTemplateColumns: typeof tmpl==='string'?tmpl:tmpl.join(' ')}}>
      {cols.map((c,i)=><div key={'h'+i} className="th">{c}</div>)}
      {rows.map((r,ri)=>r.map((c,ci)=>
        <div key={`${ri}-${ci}`} className={`td ${r.__cls||''}`} style={r.__rowStyle||{}}>{c}</div>
      ))}
    </div>
  );
};
...
const Anno = ({n, x, y, children, rot, w='160px', side='r'}) => (
  <>
    <div className="anno-mark" style={{left:x-9, top:y-9}}>{n}</div>
    <div className={`note ${side==='r'?'r':''}`} style={{
...
// Tiny inline placeholder image
const Ph = ({w='100%', h=60, label='img'}) => (
  <div className="ph" style={{width:w, height:h}}>{label}</div>
);

// Drawer / sidebar item
const NavItem = ({active, ic, children}) => (
...
const Logo = ({size=14}) => (
  <div style={{display:'flex',alignItems:'center',gap:6}}>
    <svg width={size*1.6} height={size*1.6} viewBox="0 0 24 24">
      ...
      <path d="M6 8 L6 18 M6 8 L13 8 Q15 10 13 12 L6 12 M10 14 L16 14 L18 18" stroke="#1a1815" strokeWidth="1.6" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
...
// Top app bar
const TopBar = ({user='Sara Tan', role='Project Manager', tabs, active}) => (
...
// Canonical sidebar nav per role. The Sidebar picks the right set when
// no `items` prop is passed.
const NAV_BY_ROLE = {
  PM: [
...
// Sidebar
const Sidebar = ({active='Dashboard', collapsed=false, items, user='Sara', role='PM'}) => {
  ...
  return (
    <div className="col" style={{
      ...
      <div className="grow"/>
      <div className="t-xs" style={{textAlign:'center'}}>{collapsed ? user.split(' ').map(s=>s[0]).join('') : `${user} · ${role}`}</div>
    </div>
  );
};

Object.assign(window, {
  SVGFilters, Arrow, LineChart, BarChart, Status, Table, Anno, Ph, NavItem, Logo, TopBar, Sidebar, NAV_BY_ROLE
});
